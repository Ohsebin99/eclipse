...
<a href="<%=request.getContextPath()%>/DownloadAction?file=<%=URLEncoder.encode(file.getFilename(), "UTF-8")%>&realfile=<%=URLEncoder.encode(file.getFileRealname(), "UTF-8")%>"><%=file.getFilename()%></a>(다운로드 횟수: <%=file.getDownloadCount()%>번)<br/>
