...
//	주민등록번호 앞 자리에 아무것도 입력하지 않았거나 전부 공백이 입력되었으면 에러 메시지를 출력하고
//	false를 리턴시킨다. 
	if(!obj.j1.value || obj.j1.value.trim().length == 0) {
		// 메시지
		alert('주민등록번호 앞 자리를 입력하셈');
		// 지우기
		obj.j1.value = '';
		// 포커스
		obj.j1.focus();
...
//	주민등록번호 앞 자리에 6글자가 입력되었나 검사해서 6글자가 입력되지 않았으면 에러 메시지를 출력하고
//	false를 리턴시킨다.
...
//	주민등록번호 앞 자리에 숫자만 입력되었나 검사해서 숫자만 입력되지 않았으면 에러 메시지를 출력하고
//	false를 리턴시킨다.
...
//	주민등록번호 뒷 자리를 검사한다.	
	...
	if(!obj.j2.value || obj.j1.value.trim().length == 0) {
		// 메시지
		alert('주민등록번호 뒷 자리를 입력하셈');
		// 지우기
		obj.j2.value = '';
		// 포커스
		obj.j2.focus();
...
//	자바스크립트는 문자열끼리 덧셈 연산을 하면 문자열을 연결하지만 뺄셈, 곱셈, 나눗셈은 숫자로 바꿔서
//	연산한다.
