...
//	SelectService 클래스에서 호출되는 mapper와 수정 또는 삭제할 글번호를 넘겨받고 guestbook.xml 파일의 수정
//	또는 삭제할 글 1건을 얻어오는 select sql 명령을 실행하는 메소드
...
public GuestbookList selectList(SqlMapClient mapper, int currentPage, String item) throws SQLException {
	
	return (GuestbookList) mapper.queryForObject("selectList", currentPage, item);
}

//-----------------------------------------------------------------------------------------

//	SelectService 클래스에서 호출되는 mapper와 검색어(내용)를 넘겨받고 guestbook.xml 파일의 테이블에
//	저장된 전체 글 중에서 검색어를 포함하는 글의 개수를 얻어오는 select sql 명령을 실행하는 메소드
...
//	SelectService 클래스에서 호출되는 mapper와 화면에 출력할 시작 인덱스와 끝 인덱스, 검색어(내용)가
//	저장된 Param 클래스 객체를 넘겨받고 guestbook.xml 파일의 내용에 검색어를 포함하는 1페이지 분량의
//	글 목록을 얻어오는 select sql 명령을 실행하는 메소드
