...
//	메소드
	...
	//	System.out.println("실행");
		...
		// GuestbookDAO 클래스 객체에 2번 접근해서 sql 명령을 실행해야 하므로 GuestbookDAO 클래스의 객체를
		// 미리 얻어둔다.
		...
		//	System.out.println(totalCount);
			...
			//	pageSize, totalCount, currentPage를 GuestbookList 클래스의 생성자로 넘겨선 1페이지 분량의
			...
			//	xml 파일의 sql 명령으로 전달할 데이터가 2개 이상일 경우 데이터 타입이 같다면 HashMap 객체에
			//	저장해서 전달하면 되고 데이터 타입이 다르다면 클래스 객체에 저장해서 
...
//	selectByIdx.jsp에서 호출되는 수정 또는 삭제할 글 번호를 넘겨받고 mapper를 얻어온 후 GuestbookDAO 클래스의
...
//		얻어온 글 1건을 리턴시킨다.		
	...
	public GuestbookList selectListMemo(int currentPage, String item) {
		...
		try {
			...
			// 내용에 검색어를 포함하는 1페이지 분량의 글을 얻어와서 GuestbookLsit 클래스의 ArratList에
			// 저장한다.
