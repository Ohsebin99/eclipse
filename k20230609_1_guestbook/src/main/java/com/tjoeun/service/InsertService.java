...
//	insertOK.jsp 에서 넘어오는 테이블에 저장할 실제 데이터가 저장된 객체(vo)를 넘겨받고 mapper를 얻어온 후
//	DAO 클래스의 글 1건을 테이블에 저장하는 insert sql 명령을 실행하는 메소드를 호출하는 메소드를 호출하는 메소드
	public void insert(GuestbookVO vo) {
		System.out.println("InsertService 클래스의 insert() 메소드 실행");
		
		// insert sql 명령을 실행하기 전에 필요한 전처리 작업을 실행한다. 비즈니스 로직을 실행한다.
		// ibatis mapper를 얻어온다.
		// mapper에는 데이터베이스에 연결하는 connection과 연결된 후 실행할 sql 명령(guestbool.xml)이
		// 저장되어 있다.
