...
<%		
	...
	if (guestbookList.getStartNo() > 1) {
%>
		<span class="btn" 
				title="이전 페이지"
			    onclick="location.href='?currentPage=<%=guestbookList.getCurrentPage()-10%>'">
			    이전 페이지</span>
<%
	}else {
%>
...
%>		
  <%
   		...
   		for (int i=guestbookList.getStartNo(); i<=guestbookList.getEndNo(); i++) {
  ...
  %>
  <%
	if (guestbookList.getEndNo() < guestbookList.getTotalPage()) {
%>
		<span  class="btn"
				title="다음 페이지"
			    onclick="location.href='?currentPage=<%=guestbookList.getCurrentPage()+10%>'">
