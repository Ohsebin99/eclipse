...
<!--
	currentPage를 넘겨받아 currentPage에 해당되는 1페이지 분량의 글 목록을 테이블에서 얻어와서 request
	영역에 저장한 후 1페이지 분량의 글을 브라우저에 출력하는 페이지(listView.jsp)로 넘겨준다.
...
//	게시판이 최초로 실행될 때 또는 insertOK.jsp에서 호출될 때 list.jsp로 currentPage가 넘어오지 않는다.
//	=> null
//	null은 parseInt() 메소드를 실행했을 때 NumberFormatException이 발생되므로 반드시 예외 처리를 해야한다.
//	이전 페이지에서 넘어오는 currentPage가 없으면 currentPage를 1로 초기화시켜 사용하고 넘어오는 currentPage가
//	있으면 넘어온 currentPage 값으로 초기화시켜 사용한다.
	int currentPage = 1;
	try{
...
//	검색어가 넘어온 경우(내용, 이름, 내용+이름)와 넘어오지 않은 경우에 딸근 메소드를 각각 실행한다.	
	...
	if (item == null || item.trim().length() == 0) {
		//	검색어가 입력되지 않은 경우
		//	검색어와 관계없이 1페이지 분량의 글 목록을 얻어오고 페이징 작업에 사용할 8개의 변수를
		//	초기화 시키는 메소드를 실행한다.
...
//	textarea에 글을 입력할 때 엔터키를 눌러 줄을 바꿔 입력한 경우 브라우저에 <br/> 태그로 바꿔 출력하기
//	위해 request 영역에 "\r\n"을 저장한다.
