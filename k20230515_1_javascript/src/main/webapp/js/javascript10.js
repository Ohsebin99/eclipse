...
//	reduce(): 인수로 지정된 함수를 배열에 저장된 데이터를 1개씩 너달하며 반복해 실행한다.
...
//		실행할 문장;
//		...;
//		return 값; // 현재 return 값이 다음 실행의 accmulator에 저장된다.	
//	}[, accumulator의 초기치]
// 	accumulator의 초기치를 생략하면 배열의 0번째 index의 값이 초기치가 되고 1번째 index 부터 반복한다.
...
numbers.reduce(function (acc, value, index, array) {
...
function alphaCount(alphas) {
	return alphas.reduce(function(acc, value){
		...
		if (acc[value] != undefined) { // 갹채애 value에 저장된 key가 있는가?
