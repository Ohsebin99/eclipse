...
//	변수를 선언만하고 값을 할당하지 않으면 underfined가 된다.
console.log('age: ' + age);
var age = 100;
...
//	block scope는 변수가 선언된 {} 블록 밖으로 나가면 변수가 소멸되고 function scope는 변수가 선언된
//	함수가 종료되면 변수가 소멸된다.
...
let globalName = '장길산';
{
	...
	console.log('name: ' + name); // 아무런 값도 출력되지 않는. 
...
//	template: `(그레이브)를 사용해하고 출력할 데이터는 ${ ~ } 사이에 적는다.
let count = 17.5;
const size = true;
