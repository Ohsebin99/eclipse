function numTest(num) {
	...
	console.log('123' + num); // 문자열 +문자열
	console.log(123 + num); // 숫자 + 문자열 =>문자열 연결
	console.log('123' + num); // 문자열 + 숫자 => 문자열 연결
...
//	w자바스크립트 문자열과 문자열, 문자열과 숫자, 숫자와 문자열의 덧셈은 모두 문자열 처리되므로
...
//	'100a'와 같이 인수로 지정된 문자열에 숫자로 변활할 수 없는 문자가 있을 경우 pardInt() 함수나
//	parseFloat() 함수는 변환시킬 수 있는데 까지 변환을 하지만 Number() 함수는 숫자로 변환할 수 없는
