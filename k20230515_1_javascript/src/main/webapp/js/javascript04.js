...
//	디폴트 인수 => ES6 추가
//	디폴트 인수를 사용하지 않으면 인수가 넘어오면 넘어온 값으로 함수를 실행하지만 인수가 넘어오지
//	않으면 undefined로 처리된다.
...
function printAll(...args) {
	...
	// 향상된 for
	// 향상된 for는 배열의 첫 번째 인덱스의 값을 변수에 넣고 반복을 시작하고 마지막 인덱스의 값을
	// 변수에 넣고 반복한 후 종료한다.
	for(let arg of args){
		console.log(arg);
	}
...
//	익명 함수
//	자바스크립트는 변수에 하함수를 할당할 수 있으므로 익명으로 만든 함수를 변수에 할당해서
//	사용(실행)할 수 있다. => 1급 함수
...
//	하고 어떤 이벤트가 발생했거나 특정 시점에 도달했을 때 시스템에서 호출(자동으로 실행)하는 함수를
//	말한다. => 익명 함수가 콜백을 구현할 때 주로 사용된다.
...
const simplePrint3 = () => {
	console.log('화살표 함수 simplePrint3() 실행');
};
simplePrint3();
console.log('=========================');
