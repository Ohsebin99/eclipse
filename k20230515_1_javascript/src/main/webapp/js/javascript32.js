...
//	radio button이나 checkbox의 checked 속성은 선택되면 ture, 해제되면 false를 얻어온다.
...
//	빨강, 파랑, 노랑, 검정 체크 박스가 모두 체크되었으면 전체선택 체크 박스에 체크하고 한개라도
//	체크가 안되었으면 전체선택 체크 박스의 체크를 해제한다.
	if (count == checkBoxs.length) { // 모든 체크되었나?
	...
	*/
	...
	let flag = true;
	for (let checkBox of checkBoxs) {
		//	각각의 체크 박스가 체크되어있나 검사해서 한개라도 체크가 안되어 있으면 flag를 false로
		//	변경한다.
		if (!checkBox.checked) {
			flag = false;
			//	각각의 체크 박스 중에서 한개라도 체크가 안되어있으면 나머지는 비교할 필요가 없다
			break;
		}
	}
...
//	전체선택 체크 박스를 해제하고 모든 체크 박스를 해제하는 함수를 실행한다.
	document.getElementsByName('all')[0].checked = false;
	allCheck()
}
