...
//   같은 이름으로 변수를 다시 선언했음에도 불구하고 에러가 발생되지 않고 각기 다른값이 출력되는
//   것을 볼 수 있다.
var name = 'javascript';
console.log('name: ' + name);

//   이 방식은 유연한 변수 선언으로 간단한 테스트에는 편리할 수 있으나 코드의 양이 많아지만
//   어디에서 어떻게 사용될지 파악하기 힘들고 변수에 저장된 값이 변경될 우려가 있다.
//   그래서 ES6 이후 이를 보완하기 위해서 추가된 변수 선언 방식이 let과 const 이다.

let id = 'lim'; // 변수 선언
console.log('id: ' + id);
//   let으로 선언한 id라는 변수를 같은 이름으로 다시 선언하면 이미 변수가 선언된 상태이므로 아래와
//   같은 메시지가 출력되며 프로그램이 종료된다.
...
//   let 뿐만 아니라 const를 사용해서 변수를 선언해도 같은 이름의 변수를 선언하면 변수가 이미 
//   선언되었다는 에러 메시지가 출력된다.
//   const test = 'test'; // 이미 선언한 변수를 같은 이름으로 다시 선언하면 에러가 발생된다.
//   test는 상수이므로 상수에 값을 할당하면 아래와 같은 메시지가 출력되며 프로그램이 중지된다.
//   Uncaught TypeError: Assignment to constant variable
//   test = 'check'; // 상수에 값을 할당하려 했으므로 에러가 발생된다.

//   let과 const는 변수 재선언이 되지 않는다는 공통점이 있다.
//   차이점은 let은 선언된 변수에 데이터를 재할당이 가능하지만 const는 선언시 할당된 데이터를
//   프로그램 중간에서 재할당할 수 없다.
console.log('==========================================');

//   전역 변수: 함수 외부에서 선언한 변수로 스크립트의 모든 함수에서 공통적으로 변수에 저장된
//            데이터를 사용할 수 있다.
//            var를 붙이지 않고 변수를 선언하면 전역 변수로 인식한다.
//   지역 변수: 함수 내부에서 선언한 변수 선언된 함수 내부(var) 또는 {} 블록 내부(let, const)에서만
//            변수에 저장된 값을 사용할 수 있다.
...
function scopeTest() {
   ...
   //   전역 변수와 같은 이름으로 지역 변수를 선언하면 더 이상 전역 변수를 사용할 수 없다.
   var variable = 777;
...
function test1() {
   ...
   //   innerText: 태그 내부를 의미하고 html 태그 구현이 불가능하고 모두 문자열로 처리한다.
...
function test2() {
   ...
   //   함수 내부에서 아래와 같이 var를 이용해서 전역 변수와 같은 이름의 지역 변수를 선언하면 함수 내부의 variable
   //   이라는 이름을 가지는 지역 변수로 선언되지만 var를 사용하지 않으면 전역 변수를 의미한다.
   //   지역 변수가 선언되기 전에 지역 변수를 사용하면 undefined가 출력된다. => 호이스팅
   var variable = 5;
   console.log(variable);
}
...
function test3() {
   ...
   document.getElementById('var2').innerHTML =
      '<b style="color: red; background-color: yellow;">' + variable2 + '</b>';
}
