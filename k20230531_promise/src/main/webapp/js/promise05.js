...
//	promise 만들기
//	새로운 promise가 생성되면 promise 객체를 만들때 인수로 넘겨주는
//	const promise이름 = new Promise(executor 함수);

const promise = new Promise((resolve, reject) => {
	console.log('1. promise의 executor 함수가 실행됨');
	setTimeout(() => reject('ㅎㅇ'), 2000); //	promise가 정상적으로 실행된 결과는 resolve() 함수의 인수로 리턴시킨다.
		//resolve('홍길동');
...
promise
	...
	.finally(function () {
		console.log('promise then => promise 성공, 실패 여부와 상관없이 실행된다.');
...
promise
	...
	.catch(error => console.log('1 promise then => ' + error))
	.finally(() => 	console.log('1 promise then => promise 성공, 실패 여부와 상관없이 실행된다.'))
	;
...
getHen()
	.then(hen => getEgg(hen))
//	에너가 발생된 시점에서 에러를 처리하면 발생된 시점에 catch()를 붙여 처리하면 된다.
//	.catch(function () {
	//	return '빵';
