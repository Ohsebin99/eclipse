function imgSize() {
	$('img').css({'whdth': '200px', 'height': '200px'});
}
...
//	$('img[id=selId]').css('opacity', 0.5);
//	$('img[title=img02]').css('opacity', 0.5);
//	$('img[title~=img02]').css('opacity', 0.5);
//	$('img[title*=02]').css('opacity', 0.5);
//	$('img[title$=04]').css('opacity', 0.5);
//	$('img[title^=img]').css('opacity', 0.5);
...
//   <input type="checkbok" name="chk" checked="checked"/>
//   위의 <input> 태그 요소는 checked라는 속성을 가지고 있으며 속성의 값은 "checked"이며 만약에
//   해당 체크 박스가 선택되었을 때 checked 속성의 프로퍼티는 "true"이다.
